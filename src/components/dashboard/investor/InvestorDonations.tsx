...
export const InvestorDonations = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch donations for this investor
      ...
      // Fetch verified institutions with their profiles
      ...
      if (institutionsData) {
        // Get user IDs for all institutions
        ...
        // Filter verified institutions and calculate stats
        const verifiedInstitutions = await Promise.all(
          institutionsData
            ...
            .map(async (inst) => {
              ...
              // Get donation stats for this institution
              const { data: instDonations } = await supabase
                .from("donations")
                .select("amount, status")
                .eq("institution_id", inst.id);

              const donations_received = instDonations?.length || 0;
              const total_pads_received = instDonations?.reduce((sum, d) => sum + Number(d.amount), 0) || 0;
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <Card className="p-6 bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200 hover:shadow-lg transition-shadow">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-amber-700">Total Donations</p>
              <p className="text-2xl font-bold text-amber-900">{donations.length}</p>
            </div>
          </div>
        </Card>
                    ...
                    <div className="space-y-4 flex-1">
                      ...
                      <div className="text-sm text-slate-600 p-3 bg-slate-50 rounded-lg space-y-1">
                        ...
                        {institution.contact_position && (
                          <p><strong>Position:</strong> {institution.contact_position}</p>
                        )}
                      </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                id="amount"
                type="number"
                placeholder="Enter number of pads"
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="message"
                placeholder="Add a message to the institution..."
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={submittingDonation || !donationAmount}
            >
